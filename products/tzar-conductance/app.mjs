...
function render(report){
  ...
  const invariant=sourceInvariant();
  const language=compileLanguage("tzar",{object:invariant.axisDefinition||invariant.axisTerm,subjectTrace:`${invariant.author||"автор"} · ${invariant.constructId||"конструкт"}`,innerImage:report.pass?"проводимость сохранена в предъявленных формах":"обнаружен разрыв предъявленной формы",coreNeed:"различить сохранение осевого определения",supra:invariant.axisDefinition||"авторский инвариант",nextExperiment:report.pass?"предъявить следующую форму":"вернуться к первому разрыву",riemann:"фактический ответ автора или внешнего объекта"},{subjectConfirmed:false});
...
function renderSemantic(report){
  ...
  const language=compileLanguage("tzar",{object:report.source,subjectTrace:`явно заданные опоры: ${report.anchors.join("; ")||"не заданы"}`,innerImage:hasSignals?"модель обнаружила сигналы для ручного различения":"модель построила оценку близости",coreNeed:"удержать смысловой инвариант отдельно от модельной близости",supra:report.anchors.join("; ")||"исходная авторская формулировка",nextExperiment:"провести ручной аудит отмеченных различий",riemann:"авторское решение о проводимости"},{subjectConfirmed:false});
