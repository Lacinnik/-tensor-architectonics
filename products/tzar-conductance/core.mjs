...
async function inspect(form, expected, kind) {
  ...
  return {
    ...
    kind,
    hash,
    pass: kind === "negative" ? hash !== expected : hash === expected,
    invariant,
  };
}
...
export async function verifyPayload(payload) {
  ...
  const positive = await Promise.all(forms.map((form) => inspect(form, sourceHash, "positive")));
  const negative = await Promise.all(negativeControls.map((form) => inspect(form, sourceHash, "negative")));
  return {
